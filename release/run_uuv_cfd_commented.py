...
# UUV 外流场 CFD 自动化工具
# ============================================================
# 本文件是一套可独立运行、也可被 Agent/CLI 调用的完整水动力仿真流程。
# 代码按“几何预处理 -> 自适应网格 -> 独立 Solver -> 多航速求解 ->
# 水动力提取 -> 流场后处理 -> 结构化结果输出”的顺序组织。
#
# 核心职责：
#   1. SpaceClaim：读取实体 CAD，识别模型部件，建立外流场计算域和 Named Selections；
#   2. Python：根据包围盒、面积、体积、小特征、近邻间隙等几何量计算网格尺度；
#   3. Fluent Meshing：生成面网格、边界层和体网格，并执行网格质量检查/改进；
#   4. Fluent Solver：在独立 Solver 进程中读取网格，设置材料、边界条件和监控；
#   5. 多工况求解：按用户给定航速依次计算，并分别保存 case/data 与报告文件；
#   6. 后处理：提取三方向力、压力阻力、摩擦阻力、表面压力数据和截面流场；
#   7. 结果契约：输出 Excel、PNG、原始报告、result_manifest.json 和网格质量状态，
#      便于 Claude Code / Agent 对计算状态、结果完整性和工程警告进行自动核验。
#
# 工程设计要点：
#   - 网格尺寸由模型几何尺度和局部特征驱动，不依赖固定部件名称或固定毫米阈值；
#   - 计算域相对模型真实包围盒外扩，默认以最长包围盒方向识别主流向；
#   - Fluent Meshing 与 Fluent Solver 使用独立进程，降低 Meshing Workflow 状态
#     对后续求解阶段的影响；
#   - 各航速独立写出求解结果和后处理文件，避免多工况之间互相覆盖；
#   - 关键阶段保留校验、日志和兜底路径，便于无人值守运行时定位失败位置。
# ============================================================
r"""
UUV 外流场 CFD 自动化工具
==========================
...
用途
----
对 UUV/AUV 实体 CAD 模型执行从几何预处理、外流场网格生成到多航速
Fluent 求解和后处理的自动化流程。该脚本既可直接在终端运行，也可作为
Agent Skill 的确定性底层工具，由上层 Agent 解析自然语言任务并传入运行参数。

主要流程
--------
1. 解析运行参数并确定模型路径、工作目录、航速、核数和迭代次数。
2. 调用 SpaceClaim：
   - 导入 CAD；
   - 读取各部件包围盒、面积、体积、边/面数量及局部小特征；
   - 自动构建外流场计算域；
   - 通过 Boolean 得到流体域；
   - 创建 inlet、outlet、wall、fluid 及模型表面 Named Selections。
3. 根据几何特征计算 Target Mesh Size，并配置 Curvature/Proximity、
   局部面尺寸、5 层边界层和体网格参数。
4. 使用 Fluent Meshing 生成并检查网格，将网格写为 .msh.h5。
5. 关闭 Meshing Session，启动独立 Fluent Solver 并重新读取网格。
6. 设置 water-liquid、入口/出口/对称边界、残差标准和三方向力报告。
7. 对每个请求航速依次初始化、迭代、保存 case/data，并提取：
   - Fx / Fy / Fz；
   - X 向压力阻力和摩擦阻力；
   - UUV 表面 total pressure 数据；
   - xoy / xoz 截面的速度和静压云图。
8. 汇总多航速结果，生成 Excel、力-速度关系图和 result_manifest.json。
...
推荐使用显式命令行参数：
  --model       CAD 模型路径
  --workdir     本次计算的独立结果目录
  --velocities  一个或多个航速，单位 m/s
  --cores       Fluent 并行核数
  --iterations  每个航速的迭代次数

同时支持 AUV_MODEL_PATH、AUV_WORK_DIR、AUV_VELOCITIES、AUV_CORES、
AUV_ITERATIONS 等环境变量。参数优先级由对应的解析函数定义。
...
------------
- 流体材料：water-liquid
- 默认航速：2.0、4.0 m/s（未从 CLI 或环境变量提供航速时）
- 默认并行核数：6
- 默认迭代次数：100 次/工况
- 残差绝对判据：1e-7
- 默认生成 xoy/xoz 速度与静压 PNG
- Agent/批处理模式默认在计算结束后关闭 Fluent
...
# Fluent 可执行文件路径。该路径只用于显式启动 Fluent；模型结果目录由 WORK_DIR 单独控制。
FLUENT_EXE_PATH = r"D:\Program Files\ANSYS Inc\v241\fluent\ntbin\win64\fluent.exe"

# 默认模型与默认工作目录仅作为未显式传参时的回退值；正式 Agent/CLI 调用建议始终传入 --model 与 --workdir。
# 命令行参数优先级高于环境变量，便于同一脚本在不同模型和不同结果目录之间复用。
DEFAULT_MODEL_PATH = r"D:\pyfluent\uuv.x_t"
DEFAULT_WORK_DIR = r"D:\pyfluent"

# 当输入模型不是默认模型且未指定工作目录时，WORK_ROOT 用于构造按模型名区分的回退结果目录。
# 实际 Agent 调用建议显式指定 --workdir，以避免不同任务共享同一目录。
...
def _build_runtime_arg_parser():
    ...
    同时支持显式选项（推荐）和位置参数两种输入形式。显式选项用于 Agent/批处理调用，
    位置参数作为同一工具的简化入口。这里只负责声明参数及帮助信息，不在此处执行数值校验。
    """
...
def resolve_input_model():
    ...
    优先级依次为：--model、位置参数、AUV_MODEL_PATH、DEFAULT_MODEL_PATH、
    当前目录/默认搜索目录中的可识别 CAD。返回前同时检查文件是否存在及后缀是否属于
    SUPPORTED_CAD_EXTENSIONS；若没有有效候选则抛出异常，避免后续 SpaceClaim 在错误输入上运行。
    """
    candidates = []
...
# INPUT_XT 是后续几何流程统一使用的字符串路径别名；实际输入可为任一受支持的实体 CAD 格式。
...
def resolve_work_dir():
    ...
    优先级依次为：--workdir、位置参数、AUV_WORK_DIR、默认目录规则。
    工作目录承载本次网格、case/data、报告、云图和 manifest 等全部输出，因此 Agent 调用时
    应显式传入独立目录，避免不同任务之间发生文件覆盖或结果串用。
    """
...
# ============================================================
# 阶段 A：执行 SpaceClaim 几何预处理，生成 Fluent Meshing 所需的几何文件与自适应网格输入数据。
# ============================================================
...
# 保存本次几何预处理生成的动态输出路径，后续 Fluent Meshing 直接读取这些文件。
...
# ============================================================
# 阶段 B：进入 Fluent Meshing，自适应创建局部尺寸、面网格、边界层和体网格。
# 网格策略包含局部体加密缓冲、Surface Mesh 预改善、曲率/间隙控制以及边界层过渡控制。
# ============================================================
...
# Meshing 与 Solver 使用独立 Fluent 进程，避免 Meshing Workflow/Field Mesher 状态影响求解阶段。
# Meshing 完成后通过 .msh.h5 文件交接；Solver 侧从干净 Session 重新读取网格并执行既定求解流程。
...
# 直接读取当前 SpaceClaim 阶段生成的 target mesh size 文件，并按实际部件标签建立局部网格规则。
#
...
def read_target_mesh_size_rules(path):
   """
   读取 SpaceClaim 阶段生成的 target mesh size 文件，并建立部件级网格尺寸映射。

   返回格式：
      {part_label: target_mesh_size_mm}

   解析时使用文件中的 exact part_label 与 target_mesh_size_mm 字段，自动跳过空行和以 # 开头
   的说明行。该映射是后续 Local Face Sizing、Surface Mesh 全局尺度和 Volume Mesh 尺度计算
   的统一输入，因此无法解析的尺寸不会写入网格控制。
   """
   rules = {}
...
def compute_surface_size_controls(target_size_rules, metrics_path):
   ...
   if bbox is not None:
      ...
      # Surface Mesh 最大尺寸同时受模型长度、横向尺度和主体目标尺寸约束，
      # 因而更换模型尺度后会自动同比缩放，不依赖某一具体模型的绝对尺寸。
...
def compute_volume_max_cell_length(target_size_rules, metrics_path, surf_max_size):
   """
   根据当前模型的几何尺度和部件 target size 计算体网格最大单元尺度。

   计算同时考虑模型流向长度 L、横向参考尺度 D_ref、主体/部件 target size 以及 Surface Mesh
   最大尺寸，并通过上下界约束避免体网格尺度与表面网格或模型尺度失配。返回值直接用于
   Fluent Meshing 的 Volume Mesh Max Cell Length。
   """
   h_values = [float(v) for v in target_size_rules.values() if _safe_float(v) and _safe_float(v) > 0]
   if len(h_values) == 0:
...
# 该全局容器在运行时由 build_local_face_sizings() 根据当前模型动态更新。
LOCAL_FACE_SIZINGS = []
...
# 当前主流程默认关闭 Local Refinement Regions：已验证的 Fluent 2024 R1 环境中，
# 已观察到在 local sizing / refinement task 插入阶段可能触发 Cortex SEGMENTATION VIOLATION；
# 因此当前默认禁用该功能，仅在目标工作站确认 Workflow 状态和稳定性后再启用。
# 开启该功能前应先确认 Create Local Refinement Regions 的任务对象和参数状态可正常读取。
...
# 该变量表示 Fluent GUI 中的“单元质量限制”阈值，用于网格质量改进与结果校验。
...
# 多速度批量计算设置
# ========================================================
# 航速优先从 --velocities 读取，其次读取 AUV_VELOCITIES；两者均未提供时使用 [2.0, 4.0] m/s。
# _normalize_velocity_list() 会检查航速为有限正数，并按用户输入顺序去重。
# Agent/CLI 调用无需修改源码中的 VELOCITY_LIST，直接通过命令行参数传入工况即可。
# 速度单位统一为 m/s；每个工况使用相同的求解设置和后处理流程。
# 单位：m/s
...
def build_local_face_sizings(labels):
   """
   根据当前模型的 target_mesh_size.txt 为实际部件创建 Local Face Sizing 配置。

   函数直接使用 SpaceClaim 输出的 exact part_label，不依赖 hull、shaft、propeller 等固定语义名称。
   每条配置包含 sizing 名称、对应 surface label 和目标网格尺寸，后续由 Fluent Meshing Workflow
   逐项创建局部面尺寸任务。
   """
   sizings = []
...
def add_face_local_sizing(workflow, name, labels, size):
   """
   在 Watertight Geometry Workflow 中为指定表面创建 Add Local Sizing 任务。

   Fluent 2024 R1 的该任务使用 BOISize 表示 GUI 中的 Target Mesh Size，因此本函数围绕 BOISize
   写入参数，并通过 Workflow 子任务创建、状态写入和多路径兜底保证尺寸真正落到目标表面。
   若首选接口不可用，会继续尝试兼容写法；任何失败都会打印诊断信息，便于检查 Workflow 状态。
   """
   ...
   parent_state_candidates = [
      ...
      # 兜底路径：若 BOISize 未被当前 Fluent Workflow 接受，仍先创建 Local Sizing 子任务，再尝试其它写入方式。
      {
         "AddChild": "yes",
         "BOIControlName": name,
         "BOIExecution": "Face Size",
   ...
   if isinstance(state, dict):
      ...
      # Growth Rate 必须显式写入 Local Sizing 子任务；否则 GUI 可能继续显示 Fluent 默认值。
...
def read_part_summary_metrics_for_refinement(path):
   """
   读取几何指标文件中的 PART_SUMMARY 记录，为局部体加密区域提供几何元数据。

   输出内容包含部件标签、包围盒、尺寸和对应 target size 等信息，供可选的 Local Refinement
   Regions 根据真实几何范围建立加密缓冲区。
   """
   rows = {}
   if not os.path.exists(path):
      return rows
...
def _get_create_local_refinement_task(workflow):
   """
   获取或插入 Fluent Meshing 的 Create Local Refinement Regions 任务对象。

   优先复用当前 Workflow 中已存在的任务；若不存在，则通过 Insert Next Task 相关接口尝试插入。
   函数只负责取得任务对象，不直接定义具体 refinement box 参数。
   """
   # 如果已经存在，直接复用。
   for task_name in [
...
def build_and_apply_adaptive_refinement_regions(workflow):
   """
   根据部件包围盒和局部 target size 构造两级 Local Refinement Regions。

   主体与附体采用不同的内外缓冲倍数，所有外扩尺度均由当前模型几何和 surface target size 推导，
   不依赖部件名称语义。该功能由 ENABLE_LOCAL_REFINEMENT_REGIONS 控制；在当前默认配置中关闭，
   只有在目标 Fluent 2024 R1 工作站确认 Workflow 稳定后才启用。
   """
...
def set_surface_mesh_controls(workflow):
   """
   设置 Fluent 2024 R1 Watertight Workflow 的全局 Surface Mesh 控制。

   使用目标环境实际接受的 SizeFunctions 和 ScopeProximityTo 枚举字符串配置 Curvature/Proximity，
   并写入最小/最大尺寸、曲率法向角、间隙单元数等参数。函数在写入后保留状态校验与兜底路径，
   避免使用 GUI 显示名称替代 Workflow API 的真实枚举值。
   """
   task = workflow.TaskObject["Generate the Surface Mesh"]

   # 使用 Fluent 2024 R1 实际接受的枚举字符串，避免 GUI 显示名称与 Workflow API 枚举不一致。
   primary_state = {
...
def improve_volume_mesh_quality_after_generation(meshing_session, workflow):
   """
   在体网格生成后执行网格质量检查，并尝试改善低质量单元。

   流程先读取当前最小 Orthogonal Quality，再按配置的 MESH_QUALITY_LIMIT 判断是否需要改进；
   优先执行已验证的 GUI journal 序列，同时尝试 Workflow/TUI 兜底路径。最终重新读取网格质量并
   把是否达到目标、改进前后最小值、阈值和继续策略写入 mesh_quality_improvement_status.out。

   该函数将“命令被尝试”和“质量确实改善/达到目标”分开记录。即使最终质量未达到阈值，
   只要 continue 规则允许，主流程仍可继续求解，但上层 Agent 应把该状态作为网格质量警告汇报。
   """
   ...
   # 3. 如果 Workflow 路径没有在日志中留下有效改进证据，则执行已验证的 GUI 操作序列。
   if not proof:
      try:
...
def improve_surface_mesh_if_available(workflow):
   """
   在 Surface Mesh 生成后、体网格生成前尝试执行表面网格质量改善。

   函数优先查找 Workflow 中可用的 Improve Surface Mesh 任务，并按配置执行有限次数的质量改善。
   如果当前 Fluent 环境没有暴露对应任务或接口不可用，则记录原因并继续，不阻塞后续体网格流程。
   """
...
def apply_generate_volume_mesh_tetra_settings(task, max_len, growth_rate):
   """
   将 Generate the Volume Mesh 任务配置为 tetrahedral 体网格。

   核心参数包括 Growth Rate 和 Max Cell Length。由于 PyFluent/Workflow 对同一字段可能暴露不同
   状态路径，本函数依次尝试完整 state、嵌套字段、item 和直接属性等写法，并在每次写入后尽量读取
   任务状态进行确认，确保体网格尺寸控制实际生效。
   """
   max_len = float(max_len)
   growth_rate = float(growth_rate)
   ...
   # 此处不调用 UpdateChildTasks，避免 Workflow 刷新子任务时重新应用默认尺寸并覆盖已写入参数。
   ...
   # 4. 属性赋值兜底：当 set_state/Arguments 路径不可用时，尝试直接写入 PyFluent 暴露的任务属性。
   attr_names = [
      "MaxCellLength",
      "MaximumCellLength",
      "MaxCellSize",
...
def set_volume_mesh_controls(workflow):
   ...
   # 当 VOL_FILL_TYPE 不是 tetrahedral 时，按对应 poly/hexcore 参数分支设置，同时统一使用 SURF_GROWTH_RATE。
   current_state = get_task_state(task)
...
def normalize_path_for_fluent(path):
   """
   把传给 Fluent TUI/Journal 的 Windows 路径统一转换为正斜杠格式。

   Fluent 的命令解释器会处理反斜杠转义，例如路径中的 ``\\n`` 可能被当作换行符，从而破坏文件名。
   所有写入 journal、TUI 或 Fluent 文件接口的路径均通过本函数标准化，减少 Windows 路径转义问题。
   """
...
def get_coordinate_fields_for_zone(solver_session, zone_name):
   """
   读取指定 zone/surface 上的 X、Y、Z 坐标字段。

   优先请求 x-coordinate、y-coordinate、z-coordinate 三个标量场，并与目标物理量使用相同的
   field_data 接口获取数据，使坐标与标量值保持同一数据来源。该坐标随后用于表面 Solution Data
   导出和 Python 截面云图三角剖分。
   """
...
def export_total_pressure_solution_data(solver_session, report_zones, velocity):
   ...
   # 清理同名/历史 total pressure 导出日志，避免本次运行误读残留文件。
   try:
...
def create_force_reports_tui_journal(solver_session, report_zones, report_files, report_configs):
   ...
   # 创建报告前先删除同名 report definition/report file；删除失败时继续尝试覆盖创建。
...
def create_force_reports(solver_session, report_zones, report_files):
   ...
   # 写入前删除同名结果文件，避免本次解析误读工作目录中残留的数据。
...
def _try_set_velocity_state_on_bc(bc_obj, zone_name, velocity, label):
   ...
   # 属性写入兜底：PyFluent 对同一设置可能暴露不同字段名，因此按多个候选路径依次尝试。
   attr_paths = [
...
def set_inlet_velocity_journal(solver_session, inlet_zones, velocity):
   ...
   for z in inlet_zones:
      ...
      # 优先采用 Fluent 2024 R1 可接受的 velocity-inlet TUI 交互格式。
...
def set_inlet_velocity_for_case(solver_session, inlet_zones, velocity):
   """
   为当前多航速工况写入 velocity-inlet 的入口速度，并进行状态校验。

   函数先通过 journal/TUI 设置速度，随后使用 PyFluent settings API 再次写入目标值，最后读取
   velocity-inlet state 并打印关键字段。多路径写入用于降低单一接口在 Fluent 2024 R1 中未落盘
   或字段映射不一致的风险，确保进入迭代前入口速度与当前工况一致。
   """
   ...
   # 优先通过 journal 设置入口速度；随后再用 settings API 写入并读取状态进行双重校验。
...
# ========================================================
# 5.9 后处理输出：云图、力结果与 Excel 完整性保护
# ========================================================

def write_scalar_report_out(file_path, title, value, extra_lines=None):
   """
   将单个标量结果写成统一格式的 .out 文件。

   即使某个数值未能取得，也会明确写出 not found，而不是保留上一次运行残留的数值。
   该设计便于 Agent/后处理脚本区分“真实数值”和“本次未获取到结果”。
   """
...
# ========================================================
# 5.95 表面静压 Surface Report 创建与读取
# ========================================================
...
def parse_wall_forces_transcript(text, report_zones=None):
   """
   从 Results -> Reports -> Forces 的文本输出中解析 X 向力分解。

   优先定位 ``Forces - Direction Vector (1 0 0)`` 表格的 Net 行，并把前三个数依次解释为
   Pressure、Viscous 和 Total。解析对象必须是真实 Forces 输出表，避免从无关日志数字中误取结果。
   """
   result = {"pressure": None, "viscous": None, "total": None}
...
def run_forces_panel_by_gui_macro(solver_session, report_zones, transcript_path):
   """
   通过 GUI journal 一次性执行 Results -> Reports -> Forces 操作并捕获结果。

   函数将打开 Forces 面板、选择 Wall Zones、确认 OK、写出报告等完整 GUI 操作写入同一个 journal，
   由 Fluent 一次读取执行，避免逐条 GUI/Scheme 调用在中间步骤中断。执行期间同时记录局部 transcript，
   并结合全局 transcript 和近期 force 文件寻找真实 Forces 表，供后续 Net 行解析。
   """
   ...
   # 这组命令直接参照你手动成功的 .trn。
   # 将完整 GUI 操作写入一个 journal 一次性执行，避免逐条 Scheme 调用在中间步骤提前结束。
   gui_cmds = [
...
def run_forces_panel_by_ti_menu_string(solver_session, report_zones, frp_name, transcript_path):
   """
   使用 ``ti-menu-load-string`` 向 Fluent 一次性发送交互式 Forces 命令序列。

   该路径用于处理带文件名提示等交互步骤的 TUI 操作，避免多次分段输入造成提示状态错位。
   函数返回原始输出文本，供上层 Forces 解析函数继续提取 Net 行。
   """
   zone_expr = " ".join(report_zones)
   local_transcript = str(transcript_path).replace("/", os.sep)
...
def parse_wall_forces_transcript(text, report_zones=None):
   """
   解析 Forces 输出中的 Net 行并提取 X 向压力、黏性和总阻力。

   目标 Net 行的前三个数分别对应 Pressure、Viscous、Total。解析时优先使用最后一个
   ``Forces - Direction Vector`` 表，其次检查全矢量 Forces 表以及带明确表头的 Net 行，
   以降低 transcript 中多个表格同时存在时取错结果的风险。
   """
   result = {"pressure": None, "viscous": None, "total": None}
...
# ========================================================
# Forces 结果提取：只从真实输出表的 Net 行读取压力、黏性和总力分量
# ========================================================
def _v66_nums(line):
   float_re = re.compile(r"[-+]?(?:\d+\.\d*|\.\d+|\d+)(?:[eE][-+]?\d+)?")
   return [float(x) for x in float_re.findall(str(line))]
...
def parse_wall_forces_transcript(text, report_zones=None):
   """
   从 Results -> Reports -> Forces 输出表的 Net 行读取阻力分解。

   对方向向量为 X 的 Forces 表，Net 行前三个数固定解释为：
   1) Pressure Drag_X；
   2) Viscous/Friction Drag_X；
   3) Total Drag_X。
   仅在识别到真实 Forces 表结构时返回数值。
   """
   result = {"pressure": None, "viscous": None, "total": None}
...
def run_wall_forces_transcript(solver_session, report_zones, transcript_name):
   """
   执行 Results -> Reports -> Forces，并从真实 Net 行读取 X 向力分解。

   正常情况下返回 Pressure、Viscous 和 Total；GUI 路径没有得到有效 Net 行时会尝试 TUI 兜底。
   无论解析成功与否，原始输出都会写入工作目录，便于后续人工审查和 Agent 故障定位。
   """
   if not report_zones:
...
def create_surface_pressure_reports(solver_session, report_zones, report_files):
   """
   创建 UUV 表面的静压统计 Report Definition 和 Report File。

   使用 Fluent 2024 R1 可接受的 ``surface-vertexavg`` 与 ``surface-vertexmax`` report-type，
   分别记录表面静压顶点平均值和最大值。创建前先解析真实受力表面 zone，并为每个报告建立独立输出文件。
   """
   report_zones = resolve_force_report_zones_for_solver(solver_session, report_zones)
   if len(report_zones) == 0:
      print("   没有找到 AUV 表面 zone，表面静压报告暂不创建。")
...
# ========================================================
# 主执行入口：Fluent Meshing、网格交接、Solver 求解与后处理
# ========================================================


# ========================================================
# 独立 Solver Session 启动与 Meshing 网格交接
# ========================================================

def launch_fresh_solver_from_mesh_v116(mesh_path, meshing_session=None):
   """
   从 Meshing 输出网格启动一个独立的 Fluent Solver Session。

   处理步骤：
   1. 检查 .msh.h5 是否存在且非空；
   2. 按配置关闭 Fluent Meshing Session，释放 Workflow/Field Mesher 状态和内存；
   3. 启动独立 Fluent Solver，使用当前 PROCESSOR_COUNT 和双精度；
   4. 优先通过 settings/file API 读取网格，失败时依次尝试 TUI 与 journal；
   5. 读取完成后检查 Solver zone 列表，并把可用 Solver Session 返回给主流程。

   该函数只负责 Meshing 与 Solver 之间的进程隔离和网格交接。材料、边界条件、初始化、
   迭代、多航速循环、受力报告和后处理均由后续 Solver 主流程统一设置。
   """
...
if __name__ == "__main__":
   ...
   try:
      ...
      print("\n [9/17] 导出网格...")
      # Fluent TUI/Journal 路径统一转换为正斜杠，避免 Windows 反斜杠转义（例如 \n）破坏命令字符串。
      ...
      if USE_FRESH_SOLVER_SESSION:
         ...
         # 可选人工回退路径：若关闭独立 Solver 模式，则直接从 Meshing Session 切换到 Solver。
      ...
      # KEEP_FLUENT_OPEN=True 时保留 Solver GUI 并等待人工检查；默认 Agent/批处理模式不等待交互。
      # KEEP_FLUENT_OPEN=False 时在 finally 中尽量正常退出仍存活的 Solver/Meshing Session。
      if KEEP_FLUENT_OPEN:
         try:
